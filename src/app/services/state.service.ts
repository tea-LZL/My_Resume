...
export class StateService {
  ...
  LoadingOff(renderer: Renderer2 | null = null, transGrowElems?: QueryList<ElementRef>) {
    this.loadingSubject.next(false);
    if (renderer && transGrowElems) {
      setTimeout(() => {
        if (transGrowElems) {
          transGrowElems.forEach(elem => {
            renderer.addClass(elem.nativeElement, 'grow');
          });
        }
